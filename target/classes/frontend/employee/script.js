...
window.onload = async () => {
    ...
    response = await fetch(domain + `/reimbursements/users/${result.object.id}`)
...
async function sortListDenied(){
    ...
    let response = await fetch(domain + `/reimbursements/users/${userId}`)
    let result = await response.json();
...
function populateTable(list){
    ...
    <button type="button" class="btn btn-info details-btn" data-bs-toggle="modal" data-bs-target="#staticBackdrop" 
